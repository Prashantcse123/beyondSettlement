...
class MUIDataTable extends React.Component {
  ...
  selectRowDelete = () => {
    ...
    if (this.options.onRowsDelete) {
        result = this.options.onRowsDelete(selectedRows);
    }
  ...
  render() {
    ...
    let count = options.count || displayData.length;

    return (
      <Paper elevation={4} ref={el => (this.tableContent = el)} style={{height: '100%'}}>
        ...
        {/*) : (*/}
          <MUIDataTableToolbar
            ...
            style={{borderBottom: '1px solid lightgray', boxShadow: '0 0 10px lightgray'}}
          />
        {/*)}*/}
        <MUIDataTableFilterList options={this.options} filterList={filterList} filterUpdate={this.filterUpdate} />
        <div className={this.options.responsive === "scroll" ? classes.responsiveScroll : null} style={{height: 'calc(100% - 120px)'}}>
