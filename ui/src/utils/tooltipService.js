export default class TooltipService {
  ...
  observeTooltips() {
    ...
    elements.forEach((el) => {
      const tooltipAttachId = String.random();
      const hideTipHandler = (elem) => {
        this.hideTip(elem);
  ...
  observeErrortips() {
    ...
    elements.forEach((errEl) => {
      const tooltipAttachId = errEl.getAttribute('id');
      const el = document.querySelector(`[data-error][data-tooltip-attached="${tooltipAttachId}"]`);
  ...
  showTip(el, tipContent) {
    const isError = !!el.getAttribute('data-error');
    ...
    this._to = setTimeout(() => {
      ...
      const cssClass = (isError ? 'error' : 'tooltip');
      const tooltipAttachId = el.getAttribute('data-tooltip-attached');
  ...
  calcSizeAndPosition(el, tooltip) {
    ...
    } else if ((rect.bottom + tooltipRect.height + topPadding) >= bodyRect.bottom) {
  ...
  hideTip(el) {
    ...
    const tooltipAttachId = el.getAttribute('data-tooltip-attached');
    const tooltip = document.querySelector(`[id="${tooltipAttachId}"]`);
