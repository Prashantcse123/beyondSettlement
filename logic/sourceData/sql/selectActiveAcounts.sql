...
,stl as (  -- Fetch all tradelines with pre defined conditions
          Select distinct stl.id as TradelineID, stl.nu_dse__Program__c as programid, stl.name as tradelinename, stl.nu_dse__New_Creditor__c as newcreditor, stl.nu_dse__Original_Creditor__c as originalcreditor, stl.*,
                ...
                spg.ProgramName, spg.enrolled_state__c as State_Of_Residency, spg.nu_dse__Total_Debt_Included__c as Enrolled_Debt, spg.nu_dse__total_verified_balance_included__c as VerifiedBalance,
                d.delinquency as delinquency_limit,
                COALESCE(NULL,stl.nu_dse__current_balance__c,stl.nu_dse__original_debt__c) as YTP_balance, stl.nu_dse__original_debt__c as OriginalBalance, stl.nu_dse__current_balance__c as CurrentBalance,
...
,pre_clubbed as (--For creditors who have terms, we can calculate FA
             Select  stl.tradelineid,stl.tradelinename,stl.programname,stl.programid,upper(stl.creditor) as creditor,ct.accepted_ratio, 
                     ...
                     stl.State_Of_Residency, stl.Enrolled_Debt, stl.VerifiedBalance, stl.OriginalBalance, stl.CurrentBalance, stl.nu_dse__Current_Stage__c, stl.lpoa_sent__c,
...
,final_tradelines as (Select *,case when (stl.term = CEILING(estimated_OfferAmt_TE*1.00/ Estimated_EMI)  or stl.term = stl.accepted_terms)
                               ...
                               end as Month_end_EMI, cast(CEILING(estimated_OfferAmt_TE*1.00/ Estimated_EMI) as int) as calc_terms
                        from (select c.*, 
...
select programname,tradelinename,creditor, State_Of_Residency as enrolledstate,
      avg_monthly_payment, delinquency as Account_Delinquency, fund_in_CFT,
       /*Balance, (estimated_OfferAmt_TE - estimated_OfferAmt) as Fee,*/ m0_bal, m1_bal, m2_bal, m3_bal, m4_bal,
       m5_bal, m6_bal, m7_bal, m8_bal, m9_bal, m10_bal, m11_bal, m12_bal,
        /*accepted_ratio,accepted_terms,accepted_pay,
       concat(concat(concat(cast(cast(accepted_ratio*100 as int) as varchar(100)),'%  over '), 
                     concat(cast(accepted_terms as varchar(5)), ' months with ')),
       ...
       case when  tradelinename_fa is null then 'not eligible(fa)' -- These tradelines have negative funds in their terms if we settle them
            ...
            when  credit_score < 7 and delinquency < 100 then 'not eligible(cs_d)'
            when  UPPER(creditor) in ('WELLS FARGO', 'DISCOVER','US BANK','CAPITAL ONE')
            ...
            when  UPPER(creditor) in ('AMERICAN EXPRESS','US BANK', 'ELAN FINANCIAL','KROGER')
...
from (
        Select stl.TradelineName, stl.ProgramName, stl.programid, stl.State_Of_Residency, stl.Enrolled_Debt, 
               ...
               stl.Estimated_EMI, stl.Month_end_EMI,stl.debt_type, stl.lpoa_sent__c, stl.OriginalBalance, stl.CurrentBalance,
               isnull(fa.fund_in_CFT,0) fund_in_CFT, 
               ...
               CASE when DATEDIFF(day, ISNULL(nu_dse__Last_Payment_Date__c, stl.CreatedDate),GETDATE()) < 180
				        then (coalesce(stl.accepted_terms,stl.accepted_terms,6)-1)
				        else (coalesce(stl.accepted_terms,stl.accepted_terms,6)-1)
				        end Max_Term,
                CASE WHEN DATEDIFF(day, ISNULL(nu_dse__Last_Payment_Date__c, stl.CreatedDate),GETDATE()) < 180
				            then ISNULL(fa.fund_in_CFT,0)
				            else ISNULL(fa.fund_in_CFT,0)
				        end Max_Term_Fund_Accumulation,
               isnull(fa1.MonthEnd_FA,0) MonthEnd_FA, stl.Target_Date, stl.delinquency,
               tln.createdDate as Tradeline_Last_Negotiated, Account_Number,/*stl.term_End_acc_bal_pre_charge,stl.term_End_acc_bal_post_charge,*/
       ...
       left join ( Select tl.tradelinename, tl.programid, count(remaining_fund) as total_funds,
        ...
        left join (Select stl.tradelineid , 
                          ...
                          sum(case when date(scheduledate) between date(getdate()) and last_day(DATEADD(MONTH,1,GETDATE())) then tran_amount else 0 end ) as m1_bal,
                          sum(case when date(scheduledate) between date(getdate()) and last_day(DATEADD(MONTH,2,GETDATE())) then tran_amount else 0 end ) as m2_bal,
                          sum(case when date(scheduledate) between date(getdate()) and last_day(DATEADD(MONTH,3,GETDATE())) then tran_amount else 0 end ) as m3_bal,
                          sum(case when date(scheduledate) between date(getdate()) and last_day(DATEADD(MONTH,4,GETDATE())) then tran_amount else 0 end ) as m4_bal,
                          ...
                          sum(case when date(scheduledate) between date(getdate()) and last_day(DATEADD(MONTH,6,GETDATE())) then tran_amount else 0 end ) as m6_bal,
                          sum(case when date(scheduledate) between date(getdate()) and last_day(DATEADD(MONTH,7,GETDATE())) then tran_amount else 0 end ) as m7_bal,
                          sum(case when date(scheduledate) between date(getdate()) and last_day(DATEADD(MONTH,8,GETDATE())) then tran_amount else 0 end ) as m8_bal,
                          sum(case when date(scheduledate) between date(getdate()) and last_day(DATEADD(MONTH,9,GETDATE())) then tran_amount else 0 end ) as m9_bal,
                          sum(case when date(scheduledate) between date(getdate()) and last_day(DATEADD(MONTH,10,GETDATE())) then tran_amount else 0 end ) as m10_bal,
                          sum(case when date(scheduledate) between date(getdate()) and last_day(DATEADD(MONTH,11,GETDATE())) then tran_amount else 0 end ) as m11_bal,
